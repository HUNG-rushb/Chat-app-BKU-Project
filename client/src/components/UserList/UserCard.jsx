...
const UserCard = ({ userListMessage, activeMessage, setActiveMessage }) => {
  const handleClickChooseMessage = (id) => {
    setActiveMessage(id)
  }
  return (
    <>
      {userListMessage.map((item) => (
        <div className={ item.id === activeMessage? 'user-card-container-active': 'user-card-container-inActive' } key={item.id} onClick={()=>handleClickChooseMessage(item.id)}>
